...
vmath defines 6 structs, each of which has a tiny two-character name.  In the following chart, dimensions are listed in the form %{color:red}_rows_x_columns_%.
...
When vectors are premultiplied with matrices (e.g., *V.Mul(M)*), they are treated as row vectors.  When postmultipled, (e.g., *M.Mul(V)*), they are treated as column vectors.
...
All non-method functions are prefixed with the type they return (e.g., *M3Identity*).  Methods don't have prefixes.  Functions and methods sometimes have a type suffix:
...
You can always modify a vector or matrix by accessing its struct members, but all methods and functions treat them as *immutable*XS:
